...
如果缺失的单词是重要的话，它将引发一系列事件和问题，扭曲Transformers模型的输出。我们将这个缺失的单词称为`unk`。
...
+   如果用户发送一个包含该标记且未被标记化的输入给Transformers，`unk`将会出现在生产中。

+   `unk`对于数据集来说不是一个重要的单词，但对Transformers的使用是重要的。

如果Transformers在某些情况下产生糟糕的结果，问题清单将继续增长。我们可以把`0.8`视为特定下游任务训练阶段Transformers模型的出色性能。但在现实生活中，谁希望与一个错误率达到 20%的系统一起工作呢：
...
这可能会导致错误的自然语言推理，例如Transformers被要求生成文本时产生*etext 是声明*。
...
工业 4.0 人工智能专家将更少地关注代码，更多地关注如何*展示*Transformers模型做什么。每个模型都需要一定程度的指导，而不仅仅是使用无条件的数据来模糊地告诉它做某事。
