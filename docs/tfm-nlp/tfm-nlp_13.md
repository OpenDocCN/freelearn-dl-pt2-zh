...
+   **3.2**: **命名实体识别**（**NER**）用于从社交媒体消息中提取实体以进行*第 3.4 阶段*。我们在*第十一章*中描述了 NER，*让你的数据说话：故事、问题和答案*。我们将使用 Hugging Face 的 BERT 变压器模型来执行此任务。此外，我们将使用 AllenNLP.org 的可视化工具来可视化实体和解释。
