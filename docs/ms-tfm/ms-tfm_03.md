...
`attention_mask` 是用于显示Transformers模型序列的起始和结束的 0 和 1 的掩码，以防止不必要的计算。每个分词器都有将特殊标记添加到原始序列的自己方式。对于 BERT 分词器，它在序列的开头添加了一个 `[CLS]` 标记，在序列的结尾添加了一个 `[SEP]` 标记，可以通过 101 和 102 观察到。这些数字来自预训练分词器的标记 ID。
