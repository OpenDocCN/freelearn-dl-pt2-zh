...
一种方法是对*z*应用一系列自回归变换，将其从一个简单分布转变为一个复杂分布；通过“自回归”，我们指的是每个变换利用了前一次变换和当前数据来计算*z*的更新版本。相比之下，我们上面介绍的基本 VAE 形式只有一个“变换”：从*z*到输出（虽然*z*可能经过多层，但没有循环网络链接来进一步完善输出）。我们之前已经见过这样的变换，比如*第三章*中的 LSTM 网络，其中网络的输出是当前输入和先前时间步加权版本的组合。
